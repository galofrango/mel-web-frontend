...
| Interruptor de `/panel` (D-189) | Apagado / Encendido / Desactivado | Rótulo (H4) + carril de 80×32 (`bg-secondary`, borde por `outline` para que el carril mida 80 exactos) con manija de 40×32 en `action-secondary`, sombra y check blanco. Se desplaza con `transform: translateX(40px)` — nunca con `left` (regla 2). **Todo su estado visual cuelga de `aria-checked`**, en `global.css`: el aspecto y lo que se anuncia salen del mismo atributo. No es un componente `.astro` todavía; hoy solo existe en `panel.astro` (Figma 1249:58237). |
| Globo de notas de `/panel` (D-188) | Abierto / Cerrado | `<div popover>` nativo con el formato del banner de sección (`banner-warning-bg` + borde `banner-warning-border`), 440px de ancho y `white-space: pre-wrap`. Capa superior del navegador — imprescindible: la tabla scrollea en horizontal y una caja dentro de una celda saldría recortada. Se ancla **por bordes** a su botón (`right`/`bottom`), porque al colocarlo aún no tiene medidas. |

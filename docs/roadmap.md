...
4. **Paginación de Lista con carga inconsistente entre navegadores** (D-067): funciona bien en algunos navegadores del propietario y mal en otros — sin diagnosticar todavía cuál es la diferencia real entre ellos. Ver la comprobación cross-browser añadida al flujo de publicación en `development.md`.
5. **Anterior/Siguiente del detalle de evento no respeta el rango de años del slider** (D-072, alcance explícitamente descartado): solo respeta búsqueda/localización activas. Añadirlo requeriría propagar `minYear`/`maxYear` a través de la navegación y tocar el área ya frágil del slider (punto 2 de "Pendiente de Auditoría" más arriba) — no abordar sin pedirlo explícitamente.

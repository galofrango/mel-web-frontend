...
12. **Safari no carga NINGUNA imagen hasta que el visitante interactúa con `drive.google.com`** (2026-07-28, aparcado por el propietario). Diagnóstico cerrado: el archivo entero se sirve desde un dominio de terceros, y la prevención de rastreo de Safari restringe esas peticiones hasta que ese dominio recibe una interacción como sitio principal. Reproducido y confirmado por el propietario: con las fotos sin cargar, abrió una URL de Drive suelta en Safari y **a partir de ese momento el resto del sitio empezó a cargarlas**. No es un problema de su teléfono: **cualquier visitante de iPhone con los ajustes por defecto entra y no ve ni una foto**.
   ...
   - **Idea del propietario, pendiente de viabilidad (2026-07-28)**: detectar Safari y mostrar un aviso cuyo botón provoque la interacción con Google que levanta la restricción. **Antes de construirlo hay que averiguar el mecanismo exacto**, porque de él depende que la idea funcione o no: la prevención de rastreo de Safari (ITP) bloquea *almacenamiento*, no la carga de recursos, así que que llegue a impedir cargar imágenes es raro y apunta a otra cosa — "Ocultar dirección IP a los rastreadores" (que enruta esas peticiones por el Retransmisor privado) o un bloqueador de contenido instalado. Si es ITP, un botón podría servir siempre que el visitante **interactúe** en la página de Google, no solo la abra. Si es un bloqueador de contenido, no serviría de nada. Prueba que lo decide: en un Safari donde fallen, desactivar "Evitar rastreo entre sitios" y recargar; después probar sin bloqueadores de contenido. Con esa respuesta se sabe si la idea es viable.

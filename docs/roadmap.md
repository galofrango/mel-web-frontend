...
- **Separadores verticales estandarizados en todo el sitio** (D-043): mínimo 32px a cada lado de cualquier separador entre módulos `TagWithLink` (Highlights de la home, panel del mapa); reparto regular a lo ancho completo cuando el contenido cabe, en vez de agruparse a la izquierda. Sin separador fantasma antes del primer tag.
- Pendiente de confirmación del propietario: excepción a partir de 800px de ancho para que las tags del detalle vuelvan a la izquierda como en escritorio (idea planteada, no implementada).

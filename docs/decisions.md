...
## D-259 · La interacción con las fichas: memoria del servidor, prefetch resucitado, rejilla que nace medida, cabecera de una pieza y F5 cronológico

Tras validar el propietario la tanda de red (D-258, "va todo muchísimo mejor"), tres quejas sobre las fichas: tardan unos ms en abrir incluso en escritorio; en Chrome móvil, al volver, la tarjeta de origen parece cambiar de columna; y siguen viéndose carteles por detrás de la cabecera un momento al volver. Más una decisión nueva suya: *"si ayuda de algo que el barajado activo se vaya al cuerno al recargar, nos lo ventilamos; lo de volver de una ficha es básico"*. Cinco piezas, cada una con su causa medida:

**1. Memoria del servidor para la hoja** (`fetchSheetRows`, mel.ts): se leía Google Sheets EN CADA request (~300-800ms de TTFB solo en eso). Ahora se memoriza 5 minutos por instancia — el mismo plazo que el `s-maxage`, un solo reloj de frescura en todo el sitio — y con fallo o shell rate-limitado de 3KB se sirve la copia vieja: la trampa 7 del traspaso ya no puede vaciar la web. Esto es lo que acelera la PRIMERA visita a cada ficha, que la CDN aún no tiene.

**2. `max-age=60` en vez de `max-age=0`**: con 0, el navegador revalidaba siempre, así que el `<link rel="prefetch">` que las tarjetas disparan al posar el dedo (D-161) quedaba **inutilizado** — la navegación no podía reutilizar lo precargado sin volver a la red. Con 60, la ficha precargada abre de caché local. Un F5 explícito revalida igual (los reloads se saltan el max-age).

**3. La rejilla nace medida** — el arreglo estructural de la columna que baila. El masonry esperaba al `onload` de cada imagen para conocer su proporción, y en una rejilla de 2 columnas cada medición tardía puede recolocar todo lo posterior — de ahí que al volver la tarjeta "cambiara de columna", que el scroll restaurado se moviera y que `volverAlFlyer` tuviera que perseguir a su tarjeta. Pero las proporciones **ya se sabían**: `flyer_tecnico.json` (el panel) tiene las dimensiones reales de las 101 imágenes. Ahora el SSR las adjunta a cada evento (`item.ratio`), `FlyerCard` sirve `data-ratio`, la réplica `buildGalleryCard` hace lo mismo, e `initHomePage` dimensiona TODO en síncrono antes del primer filtro (no un rAF después). Además `ratiosMedidos` pasa a vivir en `_melState`: era una `const` del script y moría en cada navegación suave — justo al volver de una ficha, cuando más falta hacía. Cobertura verificada: 61/61 eventos con ratio, 32/32 tarjetas SSR con `data-ratio`. Un cartel futuro sin auditar por el panel conserva el camino viejo (medirse al cargar).

**4. La cabecera, UNA pieza opaca en toda transición** (petición literal del propietario): los grupos sueltos (`mel-header`, `mel-toolbar`, `mel-slider`…) cubren sus elementos pero **los huecos entre ellos no eran de nadie** — por ahí se veían las tarjetas. `#bloque-cabecera` (que ya existía como envoltura opaca para el reordenado, D-XXX de nombrarHuecos) lleva ahora `view-transition-name: mel-bloque-cabecera` FIJO: grupo a z-index 99 (sobre las tarjetas, bajo sus propios hijos a 100+) y su `::view-transition-new` a 0s, para que en la vuelta —donde no tiene pareja en la ficha— aparezca opaco desde el primer fotograma en vez de fundirse dejando ver detrás. `nombrarHuecos()` lo renombra a `mel-bloque-fijo` durante el sort y ahora restaura este nombre, no vacío. El argumento antiguo ("fijo se metería en las transiciones entre páginas, donde no tiene destino") era exactamente al revés: ese grupo sin destino es lo que faltaba.

**5. F5 = cronológico** (contrato de navegación actualizado, sustituye a la regla anterior): una carga real de documento sin `mel-return-state` borra `mel-session-order`, `mel-orden-galeria` y `mel-sort-state` juntos — la página arranca como la sirve el servidor y desaparece el último caso en que el primer filtrado reconstruía las 32 tarjetas. Guardas: `!window._melState` (una navegación suave no borra nada: desincronizaría el botón de la secuencia en memoria) y la vuelta de ficha conserva todo. Verificado en navegador: `shuffle` plantado + recarga → clave borrada y secuencia resembrada 61/61.

**Verificación**: build OK, script principal parsea, cabeceras servidas comprobadas con curl, semántica del F5 probada en vivo. **Límite del entorno, medido esta vez**: el panel corre con `window.innerWidth = 0` — sin viewport no hay layout, así que TODO lo dimensional (spans de la rejilla, apilamiento de transiciones) es invisible aquí por construcción, no señal de fallo. Validación visual: el propietario, en Chrome móvil.

## D-260 · El apilamiento vivía en la página equivocada, y el ratio del servidor manda

El propietario valida D-259 en Chrome móvil ("está muchísimo mejor todo", "la página carga rapidísimo", "la entrada al evento va como un tiro") y trae vídeo con tres flecos. Dos se arreglan aquí; el tercero queda apuntado.

**1. A la ida (galería → evento) las tarjetas volaban POR ENCIMA de la cabecera** — y la causa es de manual: todas las reglas de apilamiento de las view transitions (`z-index` de mel-header, mel-toolbar, mel-slider, mel-bloque-cabecera…) vivían en el `<style>` de index.astro. En una transición del ClientRouter **manda el CSS del documento de DESTINO**: al ir hacia una ficha el documento ya es la ficha, que no tenía ninguna de esas reglas — todos los grupos caían a `z-index: auto` y mandaba el orden del documento, tarjetas incluidas. A la vuelta (documento = home) las reglas sí actuaban: por eso el fallo era solo de ida, y por eso D-257 "funcionó" solo a medias. Movidas TODAS a `global.css` (que viaja con cada página), con puntero en index.astro; las del reordenado (`html.orden-cambiando…`) se quedan, que el sort ocurre siempre con la home como documento. Lección para la regla de estilos: una regla `html::view-transition-…` que deba actuar durante una navegación tiene que estar en la hoja GLOBAL — en el CSS de una página solo actúa cuando esa página es el destino.

**2. "Abro Teckel 1 en la columna derecha y al volver está en la izquierda"** — dos fuentes de proporción en competencia. El servidor manda el ratio real del fichero (flyer_tecnico.json, D-259), pero `measureFlyerImg` lo PISABA al cargar cada imagen con el de la miniatura de Google, que redondea a su ancho servido (w700): decimales distintos → span una fila arriba o abajo → toda la colocación posterior cambia de columna, en silencio, después de cargar. Al volver, la rejilla nacía del ratio del servidor (columna izquierda) y la sesión anterior había acabado en el medido (derecha). Ahora **el ratio del servidor es la única verdad**: measureFlyerImg no pisa un `data-ratio` existente y `buildGalleryCard` prefiere `item.ratio` sobre lo medido. Lo medido queda solo para carteles sin auditar. Mismo orden ⇒ mismas columnas, siempre.
...
## D-262 · El ritmo fino: evaporación en 20ms a la ida, y la vuelta a un solo reloj

Tercer ajuste sobre la misma transición, con el propietario afinando números ("esto va mejorando"): a la ida quiere la página evaporada "como en 20ms máximo" con la franja aguantando ese tiempo antes de fundirse; y a la vuelta ve "una franjita abajo, la que termina de cubrir las cards por arriba, que llega un poco más tarde".

**La ida** va en el `<style>` de `event/[id].astro` — la mecánica de D-260 usada a favor: el CSS del documento de destino solo actúa yendo HACIA una ficha, así que allí `html::view-transition-old(*)` a 20ms (todo lo saliente, tarjetas incluidas, se evapora en un fotograma; el par del cartel en morphing también resuelve su cruce ahí y la imagen nueva manda durante todo el vuelo) y la franja con `delay: 20ms + 180ms` — aguanta opaca mientras todo muere debajo y se despide después. Bonus no pedido: Anterior/Siguiente (ficha → ficha) hereda la evaporación rápida.
...
### D-263 ter · Transparencia pura y medio segundo

Con la estructura ya validada por el propietario ("ya no se mueve nada de lo que hay debajo"), quedaba el aspecto del fundido: lo vio como "algún método de fusión de capa", y es literal — el navegador funde old y new con `mix-blend-mode: plus-lighter` dentro de un image-pair aislado, pensado para crossfades donde las dos imágenes se reparten la opacidad. Aquí la de abajo está siempre opaca, y la mezcla aditiva no parece una ficha volviéndose transparente sino una fusión rara. Arreglo: `isolation: auto` en el image-pair y `mix-blend-mode: normal` en las dos imágenes — fundido corriente de opacidad. Y la disolución sube de 180 a 500ms por orden expresa ("si dura menos de medio segundo, súbelo a esa duración"). La tapa de 120ms se queda.

**Segundo intento del propio ter, tras "lo de la luz sigue igual"**: declarar `mix-blend-mode: normal` no bastó porque el navegador aplica el `plus-lighter` MEDIANTE UNA ANIMACIÓN interna que acompaña al fundido — y una animación en marcha le gana a las declaraciones normales del autor; estirar la duración estiraba también la animación de mezcla. El único camino es no dejar correr las animaciones internas: la imagen nueva se queda sin ninguna (`animation: none`, visible a opacidad natural desde el primer fotograma) y la vieja corre una `@keyframes` propia (`mel-vuelta-desvanecer`) que solo toca la opacidad — 500ms, tapa de 120ms con `both`. Sin la animación interna, la mezcla vuelve a `normal` sola. Trampa nueva para la colección: **las view transitions aplican sus mezclas por animación, no por declaración — para quitarlas hay que sustituir `animation`, no declarar encima.**

### D-263 quater · El marcador no puede retirarse con un reloj: la ida heredaba la vuelta

Con la disolución ya en transparencia pura, el propietario reporta el morphing de ENTRADA roto. Causa: la retirada de `mel-vuelta` era un temporizador ciego de 600ms, y en el ping-pong natural de probar (entrar-volver-entrar) la ida arrancaba con la clase aún puesta — los nombres de las tarjetas apagados por el CSS de la vuelta, y sin nombre no hay pareja ni vuelo. Verificado antes de tocar que los nombres del morphing seguían intactos en el HTML de ambas páginas: lo roto era solo el residuo del marcador.
...
## D-264 · EXPERIMENTAL — inclinación 3D de las tarjetas, en la capa de dentro

Rama `experiment/tilt-3d-galeria`, aparte a propósito y sin tocar producción. Retoma el efecto que quedó encallado en julio (`experiment/gallery-3d-tilt`, problema conocido nº 4 del roadmap: dejaba de actuar al volver de una ficha) con una arquitectura distinta y con los valores que el propietario ajustó a mano en un laboratorio interactivo el 06-08-2026.

**La decisión que lo cambia todo: el giro NO va en la tarjeta.** El `transform` de `.gallery-item` ya tiene tres dueños —los vuelos FLIP del slider (D-251), las entradas y despedidas (D-256) y el morphing hacia la ficha—, y añadir un cuarto escritor en la misma propiedad es exactamente lo que hizo encallar el intento anterior (su propio mensaje de commit menciona el "bypass del FLIP reorder" entre los intentos fallidos). Ahora hay una capa interior, `.carta-3d`, que es la única que gira; la tarjeta sigue siendo de quien era. Espejo obligado en las dos copias (regla 7): `FlyerCard.astro` y `buildGalleryCard()`.

**Tres choques que hubo que resolver, cazados leyendo el código antes de escribir nada:**
1. **El recorte.** `.gallery-item` llevaba `overflow-hidden`, que habría recortado la propia capa al crecer: la escala no se habría visto y las esquinas inclinadas saldrían cortadas. El recorte se muda a `.carta-3d`.
2. **La escala duplicada.** `.gallery-item:hover` escalaba a 1.025 con `!important`; multiplicado por el 1.045 del efecto daba 1.071. Esa regla se retira y la escala pasa a la capa, en CSS (no en JS) para que funcione igual antes de que arranque el script.
3. **La sombra a rastras.** `.gallery-item` transiciona `box-shadow` 300ms, y la sombra se recalcula en cada fotograma: se habría arrastrado por detrás del ratón. Bajo `.inclinando` la transición se queda solo con `transform`.

**Y una cosa que no es un choque sino una obligación**: `navigateToEvent()` endereza la tarjeta EN SECO (sin los 700ms de vuelta) antes de navegar. En escritorio se pulsa el cartel que se está sobrevolando, o sea inclinado, y el morphing fotografía el estado viejo al arrancar: sin esto, la foto de partida sería una tarjeta torcida.

**Ajustes del propietario** (todos juntos en la constante `TILT` de index.astro y en el bloque `.carta-3d` del `<style>`; los dos números que aparecen en ambos sitios —perspectiva 600px y reposo 700ms— van avisados): inclinación 4°, perspectiva 600px, escala 1.045, sin deriva de la foto, sin suavizado, vuelta al reposo 700ms; reflejo de **barrido** al 24%, luz a 355°, banda rotada +135°, tamaño 35%, borde muy difuso, desenfoque 20px, mezcla aditiva, recorrido 8%/grado con respuesta **perezosa** (cuadrática); sombra al 48%, 8px de caída, 16px difuminada, según la luz y viva con el giro.

**Detalles de implementación que evitan los problemas conocidos**: la perspectiva va DENTRO del `transform` (como propiedad solo alcanza a los hijos directos, y así además cada tarjeta tiene su punto de fuga centrado en ella); `transform-style: flat` (con `preserve-3d` el navegador rasteriza capas por separado y deja costuras); delegación en `#gallery-grid` (cubre las tarjetas que llegan por scroll infinito, filtrado o reconstrucción al volver — que es lo que se le escapaba a la versión de julio); la caja se mide UNA vez por tarjeta y se relee al hacer scroll; `will-change` solo en la tarjeta activa, no en las 32; y puerta de entrada en `(hover: hover) and (pointer: fine)` más `prefers-reduced-motion`, así que en móvil el efecto no existe.

**Verificado en navegador**: el espejo SSR ⇄ JS es idéntico árbol a árbol tras forzar una reconstrucción con el buscador; el recorte quedó fuera de la tarjeta y dentro de la capa; la matriz de la capa es `matrix3d` con proyección −0,00167 (o sea, perspectiva activa de verdad); al entrar el puntero se pone `.inclinando`, la transición de la capa cae a 0s, la de la tarjeta se queda solo en `transform`, el brillo sube a 0,24 y `will-change` se marca solo ahí; el enderezado previo a navegar limpia clase, giros, sombra y aceleración; y la consulta de contenedor de la pastilla sigue midiendo la tarjeta. **Lo que NO se pudo probar aquí**: el efecto en movimiento — el panel de este entorno no genera fotogramas y `pintar()` corre en `requestAnimationFrame`. La validación visual y el rendimiento real en Chrome de escritorio son del propietario.

### D-264 bis · Tres remates tras la primera prueba del propietario

Validado que **no rompe nada** ("creo que no se rompe"), quedaban tres cosas:

**1. El reflejo iluminaba el cartel entero — error mío de bulto.** En el laboratorio, «tamaño 35 · borde muy difuso» produce una banda de `avance ± 35%`. Al pasarlo al sitio escribí `banda * 2`, o sea `± 70%`: **el doble de ancha**, y con la banda cubriendo 140% de la tarjeta el barrido no se leía como barrido sino como un velo. Corregido a `± banda`. La constante lleva ahora escrito que es la MEDIA anchura, que es donde me confundí.

**2. Los carteles altos destapaban la caja blanca.** No era un fallo, era geometría: al girar con perspectiva, el canto que retrocede se proyecta hacia dentro, y la escala de 1.045 da un margen que es un PORCENTAJE del alto mientras el desplazamiento crece con el alto en PÍXELES. Por eso los verticales lo agotan y los apaisados ni se acercan. Despejando la condición de cobertura (`d·(k·cos t − 1) ≥ (alto·k/2)·sen t`) sale el tope de giro de cada tarjeta, y se calcula por tarjeta en el momento de sobrevolarla — no de una tabla, porque el alto depende del número de columnas y ese número no lo decide un breakpoint sino una medición. Medido con la perspectiva y la escala actuales: a cuatro columnas el más alto del archivo admite 4,75° (ni se entera del tope), a tres 3,08° y a dos 2,37°. Los apaisados pasan de 15°. Se aplica un margen del 90% por si acaso, y el recorrido del reflejo se normaliza contra el tope real para que un cartel alto no pierda brillo solo por haberle recortado el giro.

**3. La sombra vieja asomando.** El propietario preguntó si se mantenía por algo: **no**. Yo escribía la nueva en línea sobre la de hover de siempre, y en el hueco entre que el ratón entra y llega el primer fotograma —y otra vez al salir— asomaba la de antes. Ahora la sombra del efecto ES la regla de hover (dentro del `@media` de puntero fino, así que en táctil nada cambia) y el motor solo mueve su desplazamiento con dos variables: no hay dos sombras que puedan pelearse, ni un `!important` en línea encima de otro. Comprobado en el CSS servido que la regla nueva va después de la vieja, que es lo que decide a igualdad de especificidad.

### D-264 ter · El tope de giro se calcula por ESQUINAS, no por cantos

El arreglo de D-264 bis se quedó corto y el propietario volvió a ver la caja asomando. La cuenta miraba solo los **cantos** (el borde superior o el lateral, por separado), pero el blanco sale por una **esquina**: ahí los dos giros se suman, el retroceso es mayor y la proyección se mete más adentro. Con la misma tarjeta de 291×516, por cantos salían 4° y por esquinas caben 3,36°; ese hueco era exactamente el fallo.

Ahora se proyectan las cuatro esquinas de verdad —girar en Y, luego en X, luego dividir por la perspectiva— y se busca por bisección (16 pasos) el mayor giro que las mantiene todas fuera de la caja, con medio píxel de colchón contra el redondeo. Es exacto sea cual sea la proporción del cartel, y hace innecesario el factor de seguridad a ojo del intento anterior (retirado). Medido sobre las tarjetas reales de una pantalla de cuatro columnas: los topes van de 3,22° (el más vertical) a 3,96° (los apaisados), y **las ocho cubren su caja con su propio tope** — comprobado en el navegador ejecutando la misma función contra las medidas reales. Un aviso para quien lo retome: al verificarlo, no redondear el tope antes de volver a comprobarlo; redondear hacia arriba lo empuja pasado el límite y da un falso fallo (me pasó).

## D-265 · El parpadeo al teclear en el buscador: una regla sin acotar que quemaba la cabecera

El propietario lo describe con precisión: al escribir, "la cabecera entera hace unos parpadeos, como si el fondo se volviese blanco y algunos elementos desaparecieran", una vez por tecla. Y avisa de que también pasa en producción. Lo segundo es cierto y lo primero tiene una sola causa; los "elementos que desaparecen" no desaparecen — se lavan, que es lo que él mismo confirmó al mirarlo otra vez ("solo cambia de color, se aclaran").

**La causa.** Cada tecla arranca una transición. El navegador cruza la foto vieja con la nueva **sumando luz** (`plus-lighter`), un truco pensado para que un cruce entre imágenes idénticas sea invisible: mientras una baje al mismo ritmo que la otra sube, la suma es exactamente 100%. Pero `global.css` tenía `html::view-transition-new(mel-bloque-cabecera) { animation-duration: 0s }` **sin acotar**: la imagen nueva de la franja saltaba a opacidad completa en el primer fotograma mientras la vieja seguía desvaneciéndose 250ms. Una entera más otra a medias = hasta 150% de luminosidad, durante un cuarto de segundo, en cada pulsación. De ahí el blanqueo y el texto lavado, y de ahí que afectara a la franja y no a las tarjetas: era la única con ese desajuste.

**Y no servía a nadie.** Nació en D-259 para que la vuelta de una ficha mostrara la cabecera opaca desde el primer fotograma. Pero desde D-263 la vuelta **apaga todos los nombres** de la home, así que en una vuelta ese grupo ni existe. La regla solo alcanzaba a las transiciones dentro de la home —búsqueda y salto de barra—, que es justo donde estorbaba. Retirada; el `z-index: 99` del grupo se queda, que ese sí trabaja.

El sort no estaba afectado porque allí el bloque se renombra a `mel-bloque-fijo` (`nombrarHuecos`), y la regla no le alcanzaba.

**Queda aparte** lo que el propietario intuía como segundo problema: cada tecla aborta la transición de la tecla anterior (regla 3). Con el blanqueo fuera habrá que volver a mirarlo — puede que ya no se note, y si se nota, la vía es no transicionar mientras se escribe.

**Verificado**: la regla ya no está en el CSS servido, la vuelta sigue apagando el nombre del bloque y el z-index del grupo sigue en pie. El parpadeo en sí es visual: lo confirma el propietario.

## D-266 · La vuelta al panel del mapa, y el atributo que hacía recargar TODOS los enlaces del sitio

Petición del propietario (07-08-2026): al cerrar un evento abierto desde el panel del mapa, "la página desaparece del tirón y se desencadena de nuevo la carga del mapa y el despliegue del panel en un tiempo récord, todo muy rápido y confuso". Quiere lo mismo que se consiguió en la galería: que la ficha se funda suavemente sobre el mapa **con el panel ya abierto tal cual se dejó**. Y que la tag de "Lugar" aterrice en ese mismo punto, ya que todos los eventos de esa navegación comparten local.

**Esto pisa la regla 15 de AGENTS.md** ("NUNCA ALTERAR SSR/ANIMACIONES DE ESTE FLUJO... rompe los listeners del panel lateral, dejándolo atascado", D-116 y cuatro intentos fallidos). Se avisó al propietario y decidió seguir. Va en rama aparte. Lo que cambia el cálculo respecto a 2026-07: la franja de cabecera viaja como UNA pieza desde D-261, y desde D-263 la vuelta se resuelve apagando todos los nombres y disolviendo la ficha sobre la home ya montada. El motivo original del veto —el panel llegando a destiempo detrás del slider y los tags— ya no puede darse.

**El hallazgo de fondo, que no buscaba: `Link.astro` hacía recargar la página entera en CADA enlace del sitio.** Emitía `data-astro-reload={reload}` siempre, y con el valor por defecto eso renderiza `data-astro-reload="false"`. El ClientRouter de Astro no mira el valor sino la PRESENCIA del atributo (`el.dataset.astroReload !== undefined`, comprobado en `node_modules/astro/components/ClientRouter.astro`), así que "false" contaba como "sí, recarga". Ningún sitio del proyecto pasa `reload`: nadie lo quiso nunca. Eso explica por qué llegar al mapa por la tag de "Lugar" se sentía un pantallazo, y de paso tiraba la aplicación ya cargada en cada salto por una tag (Lugar, Localidad, Diseño, Organiza, artistas). Ahora el atributo solo se emite si se pide de verdad.

**Las otras tres piezas:**
1. **El cierre deja de forzar recarga.** Se retira el `data-astro-reload` del botón de cerrar cuando hay `?location=`; la marca `mel-volviendo-al-panel` se mantiene, que es la que le dice a la home "esto es una vuelta a un estado, no un viaje a un sitio" (cámara sin viaje y panel poblado en el acto, en vez de la coreografía de un segundo).
2. **El panel se abre de verdad en el DOM nuevo — y aquí estaba el "panel atascado" de la regla 15.** La auto-apertura exige `!activeSidePanelKey`, y en una navegación suave el estado sobrevive: la clave seguía apuntando al local de antes, así que no se disparaba… pero el DOM del panel es nuevo y nace cerrado. Con recarga dura no pasaba porque el estado nacía vacío. Se limpia la clave cuando hay marca de vuelta y el camino de siempre vuelve a correr.
3. **La tag de "Lugar" marca igual que el cierre**, con un listener delegado en el documento (la fila de tags se pinta dos veces y `AdaptiveTagsRow` la rehace al medir: un listener por enlace se perdería en cada repintado). Y el `mel-vuelta` de la ficha acepta esa marca además del estado de galería, porque por "Lugar" puede no haber galería que restaurar.
...
### D-267 bis · El relato filtraba sin animación: la vuelta seguía viva

El propietario: "las cards no se animan durante el filtrado, solo desaparecen y de forma bastante abrupta". Causa exacta, y con precedente en el propio código: `filterArchives()` solo usa transición si `!restaurandoVuelta()`, y esa vuelta dura 4 segundos desde que se aterriza mientras el relato dispara a los 320ms — **siempre** caía dentro. Sin transición, el filtro ocurre en un fotograma.
...
## D-269 · Huecos de más en la galería al volver de una ficha (Safari de escritorio)

El propietario, con captura: al volver de un evento en **Safari de escritorio** «se jode el orden y espaciado de las tarjetas». La foto descarta la primera hipótesis —no es que las tarjetas se queden sin medir, porque **las imágenes salen con su alto correcto**—: lo que sobra es **hueco vertical debajo de cada una**, y eso desalinea las columnas.

**El diagnóstico.** El alto que reserva cada tarjeta en el masonry sale de `ancho_de_columna / proporción`, así que depende del ancho de columna. Y ese ancho **no es definitivo durante el arranque**: la cuarta columna no la enciende un breakpoint sino una medición (`updateHomeToolbarLayout` → `.galeria-cuatro`), y la vista puede no estar maquetada cuando se mide. Si el `row-span` se calcula con un ancho mayor del que la tarjeta acaba teniendo, reserva de más — y ese sobrante se queda para siempre, porque el único remedido que existía va colgado del CAMBIO de la clase de columnas y además se salta en silencio si el ancho da 0 (`if (ancho)`), cosa perfectamente posible con la vista sin maquetar. Que salga en Safari y no en Chrome encaja: es puro reparto de tiempos de maquetación. Y que salga al VOLVER también: en una navegación suave la rejilla llega nueva y sin la clase de cuatro columnas.

Es además una regresión probable de D-259, donde el dimensionado pasó a correr **en síncrono** durante el arranque para que la rejilla naciera colocada — justo el momento en que el ancho puede no ser el final.

**El arreglo no persigue el instante exacto, cierra la clase entera**: una pasada de remedida cuando la maquetación ya se ha asentado (doble `requestAnimationFrame`: el primero entra en el fotograma en curso, el segundo cae después de maquetar), que reescribe TODOS los `row-span` con el ancho real. No depende de que ninguna clase cambie ni de acertar con el orden de arranque. Es idempotente y solo escribe: si el ancho no cambió, reescribe lo mismo.

**Honestidad sobre la verificación**: el fallo **no se ha podido reproducir en este entorno** —solo hay un navegador Chromium y el panel no maqueta (`innerWidth` 0), de hecho la propia pasada se autoprotege y no escribe nada allí—. Comprobado lo comprobable: el código se sirve, parsea, y el guard evita cualquier escritura cuando no hay ancho. **La confirmación es del propietario, en Safari de escritorio y volviendo de una ficha.** Si el hueco persiste, el siguiente sitio a mirar es `updateHomeToolbarLayout`: que decida el número de columnas con una medición hecha antes de tiempo.


## D-270 · Las fotos llegan tarde en Safari, y al volver de una ficha las de arriba no llegaban nunca

Dos síntomas que el propietario reportó juntos y que son el mismo mecanismo: al hacer scroll rápido se ven las cajas blancas hasta que la imagen aparece encima, y **al volver de una ficha, las tarjetas que quedan por ENCIMA del cartel de origen se quedan en blanco**. Su captura lo confirma: las tarjetas están ahí, con su hueco y su fondo; lo que falta es la foto.

**La causa.** Los carteles van `loading="lazy"` salvo los ocho primeros (D-258), y cuándo pedirlos lo decide el navegador con un margen que no está en ninguna norma. **Safari es mucho más tacaño que Chrome**: pide la foto casi cuando ya la tienes delante. Y al volver de una ficha la restauración deja el scroll a media galería, así que las de arriba nunca han pasado por pantalla y nadie las ha pedido jamás.

**El arreglo** deja de depender de ese criterio: un observador propio con 1500px de margen —el mismo orden que el centinela del scroll infinito— asciende cada imagen a `eager` antes de que haga falta. No descarga nada de más (son las mismas imágenes, pedidas a tiempo) y desde D-258 salen de la caché en las revisitas. Enganchado en los cuatro sitios donde nacen tarjetas: el arranque (que es el que cubre las servidas por el SSR, incluidas las de arriba al volver), los dos repintados de `performDOMUpdates` y el lote del scroll infinito.

...
Resolver las tres incidencias reportadas en la ficha de evento (`/event/[id]`): devolver los separadores divisores a las etiquetas móviles, hacer que la foto o cartel se adapte al contenedor rellenándolo (`object-cover`), y habilitar la apertura del visor lightbox en dispositivos tablet ($\ge 480px$).
...
### Objetivo de la Sesión
Ajustar el padding de `home-highlights-tags` en la página principal para aplicarlo únicamente en el lateral derecho (`pr-[32px]`), eliminando el margen izquierdo.

### Cambios Realizados
1. **Página principal (`src/pages/index.astro`)**:
   - Actualizada la clase a `pr-[32px]` en `<AdaptiveTagsRow id="home-highlights-tags" tags={tags} class="pr-[32px]" />`.
